...
describe('BlocksService', () => {
  ...
  it('findAll calls prisma.block.findMany without orderBy when no priority', async () => {
    const list = [{ id: 1 }];

    (prismaMock.block.findMany as jest.Mock).mockResolvedValue(list);

    // no priority in dto
    const dto = {} as FindAllBlocksDto;
  ...
  it('findAll calls prisma.block.findMany with orderBy when priority is provided', async () => {
    const list = [{ id: 2 }];

    (prismaMock.block.findMany as jest.Mock).mockResolvedValue(list);

    const dto = { priority: OrderEnum.DESC } as FindAllBlocksDto;
    const res = await service.findAll(dto);
